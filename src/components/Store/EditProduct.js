...
import {updateProduct, getAllTestUnits, getProductById} from '../../api/StoreData';

export default function EditProduct({selectedProductId, open, setOpen }) {
  ...
  const submit = () => {
    ...
    updateProduct(selectedProductId, data).then(() => {
      close();
  ...
  useEffect(() => {
    ...
    getProductById(selectedProductId).then((response) => {
      ...
      setExp(response.data.expire_date);
      setModel(response.data.model);
      setMeasuringUnit(response.data.test_unit_id);
                          ...
                          >
                            ...
                            
                            {
                              allMeasuringUnit && allMeasuringUnit.map((item) => (
                                  <option
                                    value={item.id}
                                    selected={MeasuringUnit && MeasuringUnit == item.id ? 'selected' : '' }
                                  >
                                    {item.unit}
                                  </option>
                                ))
